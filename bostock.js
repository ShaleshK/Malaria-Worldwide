const duration = 250;
d3.csv("data/complete.csv").then(async data => {
  n = 20;
  barSize = 48;
  const margin = { top: 16, right: 60, bottom: 60, left: 0 };
  ...
  const datevalues = Array.from(
    ...
    .map(([date, data]) => [new Date(`${date}-1`), data])
  ...
  // const lastFrame = keyframes[keyframes.length - 2];
